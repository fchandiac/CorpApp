...
export default function TokensGrid() {
    ...
    const columns = [
        ...
        { field: 'expiration', headerName: 'Fecha expiración', flex: 1.2, type: 'date' },
        {
            field: 'actions',
            headerName: '',
        ...
        tokens.destroy(rowData.id)
            .then(() => {
                records.create(
                    'créditos',
                    'elimina',
                    'crédito ' + rowData.id + ' de ' + rowData.name,
                    router.query.userId
                )
                    .then(() => {
    ...
    return (
        ...
        <>
            <AppDataGrid rows={tokensList} columns={columns} title={'Créditos'} height='78vh' setGridApiRef={setGridApiRef} />
